...
- TVM requires LLVM for CPU codegen. We highly recommend you to build with the LLVM support on.
  ...
  - On supported platforms, the `Ccache compiler wrapper <https://ccache.dev/>`_ may be helpful for
    ...
    - Set the ``USE_CCACHE`` Cmake variable, provided by TVM's build system.
      Its default value is ``AUTO``, which indicates that TVM builds should
      attempt to use Ccache if and only if CMake is able to find the ``ccache``
      executable.
